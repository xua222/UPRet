...
#        assert self.task_config.max_words + self.task_config.max_frames <= cross_config.max_position_embeddings
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, video, video_mask=None,input_ids_aug=None,attention_mask_aug=None,video_type='feature1'):
        alpha=self.alpha
        ...
        if video_type!='feature':
            ...
            video_frame = -1
        sequence_hidden,visual_output= self.get_sequence_visual_output(input_ids, token_type_ids, attention_mask,
        ...
        if self.training:
            ...
            if self.sim_header == "Filip":
                ...
                sim_matrix,*_tmp = self.get_similarity_logits(sequence_hidden,visual_output,attention_mask,video_mask,shaped=True)
    ...
    def get_sequence_visual_output(self, input_ids, token_type_ids, attention_mask, video, video_mask, shaped=False, video_frame=-1,input_ids_aug=None,attention_mask_aug=None):
        ...
        sequence_hidden = self.get_sequence_output(input_ids,  token_type_ids, attention_mask, shaped=False)

        visual_output = self.get_visual_output(video, video_mask, shaped=True, video_frame=video_frame)

        return sequence_hidden,visual_output
    ...
    def get_similarity_logits(self, sequence_output,visual_output, attention_mask, video_mask, shaped=False, loose_type=True,is_train=True,sequence_hidden_aug=None,text_mask_aug=None):
        if shaped is False:
            attention_mask = attention_mask.view(-1, attention_mask.shape[-1])
            video_mask = video_mask.view(-1, video_mask.shape[-1])
