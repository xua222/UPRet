...
#        assert self.task_config.max_words + self.task_config.max_frames <= cross_config.max_position_embeddings
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, video, video_mask=None,input_ids_aug=None,attention_mask_aug=None,video_type='feature'):
        ...
        if self.training:
            ...
            if self.sim_header == "Filip":
                I2T_sim, T2I_sim,d_loss, *_tmp = self.get_similarity_logits(text_feat, video_feat, text_mask,
                                                                     video_mask,
    ...
    def flip_similarity_softmax(self, text_feat, video_feat, text_mask, video_mask, sim_header="meanP",pad_type=1,text_feat_aug=None,text_mask_aug=None):
        ...
        if self.training:
            ...
            #曼哈顿
            #d_loss = torch.abs(dis_video_feat.unsqueeze(2) - dis_text_feat.unsqueeze(1)).sum(-1)
            #欧几里得
            d_loss = torch.norm(dis_video_feat.unsqueeze(2) - dis_text_feat.unsqueeze(1), p=2, dim=-1)
            
            # #KL
            # kl_loss = F.kl_div(dis_video_feat.log(), dis_text_feat, reduction='batchmean')
            ...
            # js_loss = js_divergence(dis_video_feat, dis_text_feat)
        ...
        if self.training:
            return (I2T_sim * logit_scale) ,(T2I_sim * logit_scale),d_loss.mean()
        else:
    ...
    def get_similarity_logits(self, text_feat,video_feat, text_mask, video_mask, shaped=False, loose_type=False,is_train=True,text_feat_aug=None,text_mask_aug=None):
        ...
        if self.sim_header=='Filip' and is_train==True:
            if self.training:
                I2T_sim,T2I_sim,d_loss = self.flip_similarity_softmax(text_feat, video_feat, text_mask, video_mask,
                                                        sim_header=self.sim_header,text_feat_aug=text_feat_aug,text_mask_aug=text_mask_aug)
                return I2T_sim,T2I_sim,d_loss,contrastive_direction
            else:
