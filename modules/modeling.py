...
from .PDE import DistributionUncertainty
...
#        assert self.task_config.max_words + self.task_config.max_frames <= cross_config.max_position_embeddings
        ...
        self.clip = CLIP(
            ...
            context_length, vocab_size, transformer_width, transformer_heads, transformer_layers-cut_top_layer,feature_len=task_config.feature_len,
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, video, video_mask=None,input_ids_aug=None,attention_mask_aug=None,video_type='feature'):
        ...
        if self.training:
            ...
            if self.sim_header == "Filip":
                ...
                # sim_loss=sim_loss1
                if self.dual_mix!=1:
                    if self.mix_design=='balance':
                        sim_loss1=self.loss_fct(I2T_sim)*self.dual_mix+ self.loss_fct(I2T_sim.T)*(1-self.dual_mix)
                        sim_loss2=self.loss_fct(T2I_sim.T)*self.dual_mix+self.loss_fct(T2I_sim)*(1-self.dual_mix)
                    elif self.mix_design=='depart':
                        sim_loss1=self.dual_mix*(self.loss_fct(I2T_sim)+ self.loss_fct(I2T_sim.T))
                        sim_loss2=(1-self.dual_mix)*(self.loss_fct(T2I_sim.T)+self.loss_fct(T2I_sim))
                else:
                    loss_cons=0
    ...
    def get_vis(self,text_feat,video_feat):
        ...
        video_feat_np = video_feat.cpu().numpy().reshape(-1, video_feat.shape[-1])
        text_feat_np = text_feat.cpu().numpy().reshape(-1, text_feat.shape[-1])
    
        # 合并特征并降维
        combined_features = np.vstack((video_feat_np, text_feat_np))
        tsne = TSNE(n_components=2, perplexity=30, n_iter=300)
        all_features_2d = tsne.fit_transform(combined_features)
        # 分离降维后的特征
        num_video = video_feat_np.shape[0]
        tsne_video, tsne_text = all_features_2d[:num_video], all_features_2d[num_video:]

        # 绘图
        plt.figure(figsize=(12, 8))
        plt.scatter(tsne_video[:, 0], tsne_video[:, 1], c='blue', label='Video Features', alpha=0.5, marker='^')
        plt.scatter(tsne_text[:, 0], tsne_text[:, 1], c='red', label='Text Features', alpha=0.5, marker='o')
        
        plt.legend()
        plt.title("t-SNE Visualization of Combined Video and Text Features")
    ...
    def get_text_feat(self, input_ids, token_type_ids, attention_mask, shaped=False,get_hidden=True):
        ...
        if self.sim_header=='Filip'and get_hidden==True:
            text_mask,sequence_hidden,sequence_cls = self.clip.encode_text(input_ids,return_hidden=True)  # B,512

            sequence_hidden=sequence_hidden.float()
            sequence_cls=sequence_cls.float()
            return text_mask,sequence_hidden,sequence_cls
        else:
            sequence_hidden = self.clip.encode_text(input_ids).float()#B,512
    ...
    def get_video_feat(self, video, video_mask, shaped=True, video_frame=-1,get_hidden=True):
        ...
        if video_frame!=-1:
            if self.sim_header == 'Filip' and get_hidden==True:
                video_mask,visual_hidden,visual_cls = self.clip.encode_image(video, return_hidden=True, video_mask=video_mask,video_frame=video_frame)
                ...
                visual_hidden = self.clip.encode_image(video, video_mask=video_mask,video_frame=video_frame).float()
        else:
            visual_hidden = self.clip.mlp(video.half()).float()
        visual_hidden = visual_hidden.view(bs_pair, -1, visual_hidden.size(-1))
    ...
    def flip_similarity_softmax(self, text_feat, video_feat, text_mask, video_mask, sim_header="meanP",pad_type=1,text_feat_aug=None,text_mask_aug=None):
        ...
        # #pdb.set_trace()
        if self.training:
            B_t,N_t,C = text_feat_aug.shape
            shift_size = C // 4
            txt_mu,txt_logsigma,_ = self.dist_text_trans(text_feat_aug,weight=None)
            #txt_mu,txt_logsigma = text_feat_aug,text_feat_aug
            samples = [txt_mu]

            for _ in range(self.sample_num-1):
                eps = torch.randn(B_t,N_t,C,device=txt_mu.device)
                sample = txt_mu + torch.exp(txt_logsigma) * eps
                samples.append(sample)

            dis_text_feat = torch.stack(samples)
            dis_text_feat = dis_text_feat.permute(1,2,3,0) # bs T D 2
            dis_text_feat = self._mean_pooling_for_similarity_sequence(dis_text_feat, text_mask).permute(2,0,1)# 2 bs D
            #dis_text_feat = dis_text_feat.mean(dim=1).permute(2,0,1)

            B_v,N_v,C = video_feat.shape
            vid_mu, vid_logsigma, _ = self.dist_video_trans(video_feat, weight=None)
            #vid_mu,vid_logsigma = video_feat,video_feat
            samples = [vid_mu]
            for _ in range(self.sample_num-1):
                eps = torch.randn(B_v, N_v, C, device=vid_mu.device)
                sample = vid_mu + torch.exp(vid_logsigma) * eps
                samples.append(sample)
            dis_video_feat = torch.stack(samples) # S,B,N,C
            dis_video_feat =dis_video_feat.permute(1,2,3,0) # S,B,N,C-> # B,N,C,S
            dis_video_feat = self._mean_pooling_for_similarity_visual(dis_video_feat, video_mask).permute(2,0,1) # B,C,S->S,B,C
            #dis_video_feat = dis_video_feat.mean(dim=1).permute(2,0,1)

            #cos
            dis_text_feat= F.normalize(dis_text_feat,dim=-1)
            dis_video_feat = F.normalize(dis_video_feat,dim=-1)
            
            sim = torch.einsum('vad,tbd->vtab',dis_video_feat,dis_text_feat).contiguous()# S bs D * S bs D
        
            sim = sim.view(self.sample_num,self.sample_num,B_v*B_t)
            sim = sim.permute(2,0,1) # [a*b,s,s]
            wdist = 1.0-sim # 代价矩阵

            xx=torch.zeros(B_v*B_t, self.sample_num, dtype=sim.dtype, device=sim.device).fill_(1. / self.sample_num)
            yy=torch.zeros(B_v*B_t, self.sample_num, dtype=sim.dtype, device=sim.device).fill_(1. / self.sample_num)
            ...
            sim_ot = T*sim
            sim_ot = sim_ot.contiguous().view(B_v,B_t,self.sample_num,self.sample_num) # a b v t
            t2v_sim_ot ,max_ids1 = sim_ot.max(dim=-2) 
            v2t_sim_ot ,max_ids2 = sim_ot.max(dim=-1)
            t2v_sim_ot = torch.sum(t2v_sim_ot,dim=2) / self.sample_num # a b
            v2t_sim_ot = torch.sum(v2t_sim_ot,dim=2) / self.sample_num # a b

            sim_ot = (t2v_sim_ot + v2t_sim_ot)/2.0
        ...
        if self.training:
            return (I2T_sim * logit_scale) + (self.ot_weight * sim_ot) ,(T2I_sim * logit_scale) + (self.ot_weight * sim_ot)
        else:
    ...
    def flatten_features(self,features):
        ...
        batch_size, num_elements, feature_dim = features.size()
        return features.view(-1, feature_dim), batch_size, num_elements
