...
from modules.modeling_dis import CLIP4Clip
...
def get_args(description='CLCL on Retrieval Task'):
    ...
    #output
    parser.add_argument("--output_dir", default='distance ', type=str,
...
def set_seed_logger(args):
    ...
    logger = get_logger(os.path.join(args.output_dir, "l2distance.txt"))
...
def train_epoch(epoch, args, model, train_dataloader, device, n_gpu, optimizer, scheduler, global_step, local_rank=0):
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        loss,d_loss = model(input_ids, segment_ids, input_mask, video, video_mask,pairs_text_aug,pairs_mask_aug)
        ...
        if (step + 1) % args.gradient_accumulation_steps == 0:
            ...
            if global_step % log_step == 0 and local_rank == 0:
                logger.info("Epoch: %d/%s, Step: %d/%d, Lr: %s, Loss: %f,D_loss: %f, Time/step: %f", epoch + 1,
                            args.epochs, step + 1,
