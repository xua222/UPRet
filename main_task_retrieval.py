...
def get_args(description='CLCL on Retrieval Task'):
    ...
    parser.add_argument('--features_path', type=str, default='/comp_robot/caomeng/code/SLRT/CiCo/CLCL/sign_feature/h2s_domain_agnostic', help='feature path')
    parser.add_argument('--features_path_retrain', type=str, default='/comp_robot/caomeng/code/SLRT/CiCo/CLCL/sign_feature/h2s_domain_aware', help='feature path')
...
def set_seed_logger(args):
    ...
    logger = get_logger(os.path.join(args.output_dir, "ot_csl.txt"))
...
def train_epoch(epoch, args, model, train_dataloader, device, n_gpu, optimizer, scheduler, global_step, local_rank=0):
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        iter_start_time = time.time() #批次开始时间
    ...
    average_iter_time = (total_iter_time / num_iters) if num_iters >0 else 0
    if local_rank == 0:
        logger.info(f"First iteration time: {first_iter_time}")
        logger.info(f"Average iteration time (excluding first iteration): {average_iter_time}")
    ...
    with torch.no_grad():
        ...
        for bid, batch in enumerate(test_dataloader):
            ...
            if multi_sentence_:
                ...
                mask,visual_output,visual_cls=model.get_video_feat(video, video_mask,shaped=True, video_frame=1,get_hidden=True)
                ...
                if len(filter_inds) > 0:
                    ...
                    text_mask,sequence_output, sequence_cls= model.get_text_feat(input_ids, segment_ids, input_mask,get_hidden=True)
                    batch_sequence_output_list.append(sequence_output)
                    batch_sequence_output_list_cls.append(sequence_cls)
                    batch_list_t.append(text_mask)
                total_text_num += b
            else:
                sequence_output, visual_output = model.get_text_video_feat(input_ids, segment_ids, input_mask, video, video_mask, shaped=True)
        ...
        for epoch in range(resumed_epoch, args.epochs):
            ...
            if args.local_rank == 0:
                ...
                if best_score <= R1:
                    best_score = R1
                    best_epoch=epoch
                    best_output_model_file = save_model(epoch, args, model, optimizer, tr_loss, type_name="")
